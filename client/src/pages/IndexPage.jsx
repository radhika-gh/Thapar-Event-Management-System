  ...
  return (
    <>
      <div className="flex flex-col min-h-screen">
        ...
        {/* Events grid */}
        <div className="flex-grow mx-10 my-5 grid gap-x-6 gap-y-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 sm:mx-5">
          {events.length > 0 &&
            events.map((event) => {
                    ...
                    <div className="rounded-tl-[0.75rem] rounded-tr-[0.75rem] rounded-br-[0] rounded-bl-[0] object-fill aspect-16:9">
                      {event.image && (
                        <img
                          src={`${event.image}`}
                          alt={event.title}
                          className="event-image rounded object-fill"
                        />
                      )}
